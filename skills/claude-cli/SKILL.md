...
## 🔥 peer 起套娃 codex 的硬规则(实测踩坑总结)

peer 在自己的工作流里通过 Bash tool 调 `agent codex new` 起 codex 子任务时,**必须遵守两条红线**——违反任一条,长任务的 codex 会被连带杀(实测验证过)。

### 红线 1:绝对不用 `run_in_background: true`

```jsonc
// ❌ 错误 — Claude Code Bash tool 的 background 模式
{
  "command": "agent codex new ...",
  "run_in_background": true   // ← 这个会把 codex 起到 Claude Code 的 bg manager
}
```

**为什么不行**:`run_in_background: true` **不是真后台 fork**,是 Claude Code 自己维护的 background process pool,peer 进程退出时会把里面的任务**一起清理掉**。实测:codex sleep 90s 任务,peer 11 秒退出,codex 启动 8 秒就被杀,产出文件根本没生成。

### 红线 2:用同步 bash + shell `&` 控制并行/后台

```bash
# ✅ 正确 — run_in_background: false + 命令本身用 shell & 控制
agent codex new task-a "..." -C ../task-a &
agent codex new task-b "..." -C ../task-b &
agent codex new task-c "..." -C ../task-c &
wait    # 等所有完成
```

bash `&` 是真正的 fork detach,子进程被 init 收养,**peer 退出也杀不掉**。实测验证 ✓。

### 三种典型场景

| 场景 | 命令模板 | 行为 |
|---|---|---|
| **单 codex 同步** | `agent codex new task "..." -C ...` | peer 等 codex 完,自然 |
| **并行 N 个 codex 全部等完** | `agent codex new a "..." -C ... & ; agent codex new b "..." -C ... & ; wait` | 并行跑,peer 等所有完成后整合(时间 = max,不是 sum) |
| **并行 N 个不等就退**(用户手动接力) | `agent codex new a "..." -C ... & ; agent codex new b "..." -C ... &`(**不加 wait**) | peer 立刻 final,codex 后台跑;用户主动 `agent ls` 看完成情况后续聊 peer |

> 不管哪种模式,Bash tool 都用 `run_in_background: false`(同步)。"后台"靠 shell `&` 不靠 Bash tool 选项。

### claude wrapper 不杀 peer(已关 watchdog)

考虑到 peer 经常要 `wait` 多个长跑 codex(可能等 10 分钟以上),`agent claude new/c` wrapper **不起 watchdog**——peer 真卡死靠用户 Ctrl-C 兜底,不会被自动 kill。codex 那边的 watchdog 保持,工具型任务该有保护就有。
