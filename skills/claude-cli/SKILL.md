...
description: 用 claude CLI(外部 Claude 实例,非交互 -p 模式)起独立任务 session。
  外部 claude 是你(Claude Code)的**协作 peer**——能跨项目读代码、出双边修改方案、追踪进度,
  需要写代码时自己调 codex 落地(允许)。支持多 session 并行、按 name 续聊、本地存档。
  触发词:"让 claude 帮我看一下"、"起一个 claude 协助 X"、"用 claude 跨项目协调 A 和 B"、
  "claude 出方案对比"、"让 claude 接管 X 任务"、"接着上次 claude 那个 X 继续"。
  当用户想"找一个独立 claude 协作伙伴(而不只是工具)处理某个独立工作流"时使用。
  纯写代码/重构/补测试型任务直接用 `codex-cli` skill(不必绕 claude 中间一道)。
...
跟 codex 当"工具"用不一样——**外部 claude 是 Claude Code 的平级协作者**,能独立扛一段工作流。适合派给它的场景:

- **跨项目协调**:本地有 A 和 B 两个项目,A 改了共用表结构需要 B 配合改代码。让外部 claude 直接读 B 项目代码,出 A+B 双边修改方案,追踪两边进度
...
- 单纯实现一个新函数/模块、加测试、补类型、修小 bug——这些任务没必要绕 claude 一道,直接 `agent codex new`

## 🔥 工作目录必须显式切到目标项目(`-C` 参数)

跨项目协作的**第一硬规则**:claude 的 cwd 决定它能读什么。**Claude Code 自己的 cwd 跟 claude 任务的 cwd 通常不是一回事**。

典型场景:
- 你(Claude Code)当前 cwd 在 `~/project/A`(用户跟你聊的就是 A 项目)
- 任务需要 claude 去看 B 项目代码(`~/project/B`)出双边方案
- ❌ 错误:`agent claude new ...` 不传 `-C`,claude 起来的 cwd = A,看不到 B 的代码
- ✅ 正确:`agent claude new <name> "<desc>" "<prompt>" -C ~/project/B`,claude 起来的 cwd = B,直接能读 B 的代码
...
## claude 自己不写代码,需要落地时让它调 codex

claude 在出方案 / 跨项目分析过程中**不直接动手写代码**。它的方案里如果包含要落地的实现部分,**让 claude 自己起 `agent codex new` 把代码任务交给 codex 干**——这样 claude 全程协调跟进,你(Claude Code)只需要关心顶层进展。

典型链路(A 改表结构 + B 跟改代码):
1. claude 读 A 的 migration、读 B 的 ORM/调用方代码 → 出双边方案(A 改什么、B 改什么)
2. 你(Claude Code)看完方案,觉得 OK → 让 claude 继续推进
3. claude 起 `agent codex new` 在 B 项目目录里让 codex 按方案改 B 代码
4. claude 收 codex 的执行结果 → 回报给你
5. 你审 codex 的 diff、commit、回报用户

> codex 不会自己再起新 agent session(safety 约束),所以最多到 claude → codex 两层,不会无限套娃。
...
- **[产出]** 形式(方案对比表/清单/利弊分析)+ 长度(如 "500 字内")。**Claude Code 自己也不确定要什么时,这项可以省**——让 claude 自己判断更合适的产出形式
...
## 要改代码:claude 自己起 codex,不直接动手

claude 出完方案后,如果需要落地改代码,**让 claude 起 `agent codex new` 把任务交给 codex**,不要 claude 自己动手编辑文件。理由:

1. **codex 编码能力更强**(gpt-5.5 + xhigh + 沙箱化执行)
2. **职责分层清晰**:claude 负责"想 + 协调",codex 负责"动手 + 改文件"
3. **worktree 隔离也是 codex 负责**:claude 起 codex 时按 `codex-cli` skill 的 worktree 流程,加 `-C ../foo-codex-<name>` 让 codex 在独立 worktree 改文件

claude 起 codex 的标准模板(假设 claude 在 cwd /Users/x/project/foo 协调 A+B):

```bash
# claude 在 A 项目里出完方案,需要 B 项目跟改 → claude 自己跑:
cd /Users/x/project/foo-b
git worktree add ../foo-b-codex-update-orm -b codex-update-orm
agent codex new update-orm "按 claude 出的方案更新 B 项目的 ORM 模型" \
  -f /tmp/agent-prompt-update-orm.md \
  -C ../foo-b-codex-update-orm
```

codex 改完文件后输出回到 claude,claude 整合结果继续往下推。
