...
外部 claude peer 默认走 `~/.claude/settings.json`(当前:`opus[1m]` 长上下文 + bypassPermissions;思考强度未配置,走 CLI 默认)。底层主推 `agent peer new/c` 子命令；`agent claude new/c` 是完全等价的兼容别名，老用法不破坏。
...
- ❌ 错误:`agent peer new ...` 不传 `-C`,claude 起来的 cwd = A,看不到 B 的代码,更没法改 B
- ✅ 正确:`agent peer new <name> "<desc>" "<prompt>" -C ~/project/B`,claude 起来的 cwd = B,既能读 B 的代码也能直接改 B 的文件
...
## peer 内部起 codex 的姿势——你不用管

peer 在自己工作流里通过 Bash tool 起 `agent codex new` 子任务时,有几条姿势硬规则(用 shell `&` 不用 `run_in_background:true`、并行用 `& + wait` 等)。

这些规则**已经在 wrapper 层面通过 prompt 注入自动告诉 peer**,你(主对话)起 peer 时**不需要在 prompt 里写**——peer 收到的 prompt 末尾自动有这条注入。

考虑到 peer 经常要等多个并行 codex(可能 10 分钟以上),`agent peer new/c` wrapper **不起 watchdog**——peer 真卡死靠用户 Ctrl-C 兜底。codex 那边 watchdog 仍保留,工具型任务有保护。
...
| 场景 | 命令 |
|---|---|
| 新起 session | `agent peer new <name> <desc> (<prompt> \| -f file) [-m M] [-e E] [-C dir]` |
| 续聊 session | `agent peer c <name> (<prompt> \| -f file) [-m M] [-e E] [-C dir]` |
| 列出所有 session | `agent ls [type]`(可按类型过滤；peer session 前缀仍是 `claude-`) |
...
## 不追加 safety suffix（但会注入 peer 嵌套规则）

**`agent peer new/c` 不追加任何 safety suffix**。wrapper 只在末尾注入 peer 使用 codex 子任务时的嵌套规则；claude 仍被当作"协作者"使用，完整能力不受限——包括 git commit/push、起新 agent session 等都不自动禁止。
...
# 2. 起 session
agent peer new eval-cache "<desc>" -f ~/tmp/agent-prompt-eval-cache.md -C ~/project/myrepo
```
...
❌ 不假设跨 session 共享上下文:
```bash
agent peer new plan-a "<desc>" "评估方案 A: ..."
agent peer new plan-b "<desc>" "评估方案 B,跟 A 对比"        # B 不知道 A 是啥
```

✅ 各 session prompt 自包含:
```bash
agent peer new plan-a "<desc>" "[目标] 评估方案 A: <完整描述>"
agent peer new plan-b "<desc>" "[目标] 评估方案 B: <完整描述>。与 A 的差异:<列出来>"
...
❌ 续聊时用模糊指代:
```bash
agent peer c x "继续刚才那个"
```

✅ 续聊写新增信息 + 具体指令:
```bash
agent peer c x "你方案 A 里说要用 Redis Pub/Sub,但我们的运维不支持 Pub/Sub。换个等价方案。"
```
...
```bash
agent peer new plan-a "评估方案 A" "[目标] 评估方案 A: <完整描述>。[约束] 只分析。[产出] 优劣表,300 字内。" &
agent peer new plan-b "评估方案 B" "[目标] 评估方案 B: <完整描述>。[约束] 只分析。[产出] 优劣表,300 字内。" &
agent peer new plan-c "评估方案 C" "[目标] 评估方案 C: <完整描述>。[约束] 只分析。[产出] 优劣表,300 字内。" &
wait
...
| 错误 | 修复 |
|---|---|
| `❌ session 'claude-x' 已存在` | 续聊 `agent peer c x "..."` / 重置 `agent rm claude-x && agent peer new x "..." "..."` |
| `❌ session 'claude-x' 不存在` | 用 `agent peer new x "<desc>" "..."` 新起 |
...
| claude 卡很久不返回 | 长任务 1-5 分钟正常(思考强度调高会更久)。claude **不起 watchdog**(协作 peer 要等套娃 codex),真卡死靠用户 Ctrl-C |
| `❌ -m 不支持 'xxx'` / `❌ -e 不支持 'xxx'` | 传了白名单外的值,按报错提示的可选值改;要用别的值得改 `~/.claude/settings.json` |
...
**默认绝对不传** `-m` / `-e`。不传 = 走 `~/.claude/settings.json`(当前 `opus[1m]`),这是绝大多数情况下正确的做法。

**只有用户明确要求换档时才传**——比如用户说"用 sonnet 跑就行"、"这个要高思考"。**不要自己按任务性质替用户决定用什么模型/思考强度**:成本和耗时的取舍是用户的决策,不是你的。

### 可选值(白名单,传别的直接报错)

| flag | 可选值 | 说明 |
|---|---|---|
| `-m` | `opus`、`sonnet`、`fable` | 不传 = config 默认(`opus[1m]`) |
| `-e` | `low` `medium` `high` `xhigh` `max` | 不传 = config 默认 |

**三个模型都自动解析成 1M 长上下文变体**(`opus` → `opus[1m]`,`sonnet` → `sonnet[1m]`,`fable` → `fable[1m]`)。你只需要传裸 alias,wrapper 会做映射——这样"显式指定模型"不会把上下文窗口从 1M 缩回标准档。

传白名单外的值(比如 `haiku`、`claude-opus-4-8` 全名)会被本地拦下并列出可选值。用户要用别的模型 → 让他改 `~/.claude/settings.json`。

### 续聊会自动沿用,不用重复传

`new` 时传的 model/effort 会记进 session(`.ai-sessions/claude-<name>/model` 和 `effort`),**续聊自动沿用同一档**——不会出现"首轮 sonnet、续聊悄悄换回 opus"的跨轮换脑子。续聊显式传则覆盖,并成为该 session 之后各轮的新默认。

```bash
# 用户明说用 sonnet
agent peer new eval-x "评估缓存方案在低延迟场景下的可行性" "<prompt>" -m sonnet -e high

# 续聊不用重复传,自动还是 sonnet + high
agent peer c eval-x "运维说不支持 Pub/Sub,换个等价方案"
```
