...
外部 claude peer 默认走 `~/.claude/settings.json`(当前:`opus[1m]` 长上下文 + bypassPermissions;思考强度未配置,走 CLI 默认)。底层用 `agent claude new/c` 子命令(详见下方命令清单)。
...
考虑到 peer 经常要等多个并行 codex(可能 10 分钟以上),`agent claude new/c` wrapper **不起 watchdog**——peer 真卡死靠用户 Ctrl-C 兜底。codex 那边 watchdog 仍保留,工具型任务有保护。
...
| 场景 | 命令 |
|---|---|
| 新起 session | `agent claude new <name> <desc> (<prompt> \| -f file) [-m M] [-e E] [-C dir]` |
| 续聊 session | `agent claude c <name> (<prompt> \| -f file) [-m M] [-e E] [-C dir]` |
...
agent claude c eval-x "运维说不支持 Pub/Sub,换个等价方案"
```
