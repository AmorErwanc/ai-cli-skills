...
- 系统性坑 → 修改**仓库里的** `profiles/<type>/inject.md`，写明来源场景并提交；然后重跑 install（或手动拷贝到 `~/.ai-cli-skills/profiles/<type>/inject.md`）让运行副本生效，再重跑任务
- 一次性事故 → 直接重试，不动规则
- 新规则标注来源场景；场景消失（依赖升级/坑已根治）就删规则，不留墓碑
- 提示词修改永远以仓库文件为事实源并走 git 提交，不做无人值守的自动改写；直接改运行副本会被下次 install / update 覆盖
